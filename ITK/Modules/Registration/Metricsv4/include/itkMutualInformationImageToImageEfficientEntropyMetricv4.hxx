 ...
 *
 *=========================================================================*/
#ifndef itkMutualInformationImageToImageEfficientEntropyMetricv4_hxx
#define itkMutualInformationImageToImageEfficientEntropyMetricv4_hxx
...
MutualInformationImageToImageEfficientEntropyMetricv4<
  TFixedImage,
  TMovingImage,
  TVirtualImage,
  TInternalComputationValueType,
  TMetricTraits>::MutualInformationImageToImageEfficientEntropyMetricv4()
...
template <typename TFixedImage,
          ...
          typename TMetricTraits>
void
MutualInformationImageToImageEfficientEntropyMetricv4<TFixedImage,
                                                    TMovingImage,
                                                    TVirtualImage,
                                                    TInternalComputationValueType,
...
template <typename TFixedImage,
          ...
          typename TInternalComputationValueType,
          typename TMetricTraits>
typename MutualInformationImageToImageEfficientEntropyMetricv4<TFixedImage,
                                                             TMovingImage,
                                                             TVirtualImage,
                                                             TInternalComputationValueType,
                                                    ...
                                                    TMetricTraits>::ComputeValue() const
{
  ...
  /** TInternalComputationValueType px, px1, py, py1, pxy, pxy1, pxy2, pxy3, entx, enty, entj, total_mi1, Mx, My, Mj;
  ...
  //---------------------------------------------ORIGINAL FORMULA ENDS-----------------------------------------------------------------------------------------//
 
//---------------------------------------------MODIFIED FORMULA STARTS--------------------------------------------------------------------------------------//
entx = 0;
enty = 0;
entj = 0;

for (SizeValueType ii = 0; ii < m_NumberOfHistogramBins - 1; ii += 2)
{
  MarginalPDFIndexType mind;
  mind[0] = ii;
  px = this->m_FixedImageMarginalPDF->GetPixel(mind);
  ...
  mind[0] = ii + 1;
  px1 = this->m_FixedImageMarginalPDF->GetPixel(mind);
  py1 = this->m_MovingImageMarginalPDF->GetPixel(mind);
 ...
 // {
    //Mx = px + px1;
    entx += -1.0 * ((px + px1) * std::log((px + px1) / 2));
//  }

 // if (py > eps || py1 > eps)
 // {
   // My = py + py1;
    enty += -1.0 * ((py + py1) * std::log((py + py1) / 2));
 // }

  for (SizeValueType jj = 0; jj < m_NumberOfHistogramBins - 1; jj += 2)
  {
    index[0] = ii;
    index[1] = jj;
    pxy = m_JointPDF->GetPixel(index);
    index[1] = jj + 1;
    pxy1 = m_JointPDF->GetPixel(index);
    index[0] = ii + 1;
    index[1] = jj;
    pxy2 = m_JointPDF->GetPixel(index);
    index[1] = jj + 1;
    pxy3 = m_JointPDF->GetPixel(index);
   ...
   // if (pxy > eps || pxy1 > eps || pxy2 > eps || pxy3 > eps)
    //{
     // Mj = pxy + pxy1 + pxy2 + pxy3;
      entj += -1.0 * ((pxy + pxy1 + pxy2 + pxy3) * std::log((pxy + pxy1 + pxy2 + pxy3) / 4));
   // }
  }
}
total_mi1 = entx + enty - entj;
return (-1.0 * total_mi1 / this->m_Log2);
//return (-1 * total_mi1);
//} // over jh bins 2
//}   // over jh bins 1
// return (-1.0 * total_mi.GetSum() / this->m_Log2);
}
//---------------------------------------------MODIFIED FORMULA ENDS--------------------------------------------------------------------------------------//
